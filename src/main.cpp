...
#if defined(NDEBUG)
# error "Jipcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6ef84a5d173aa7666ca6cc48aba592f5ac18f498331df34cf52c311540391402");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Jipcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Jipcoin Signed Message:\n";
...
{
    // Jipcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Jipcoin: 100k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.35 * 24 * 60 * 60; // Jipcoin: 0.35 days
static const int64 nTargetSpacing = 15; // Jipcoin: 15 sec
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Jipcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jipcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x657ac0e845c6e4a895aaaa497b128c3564a954ce8a08524d6895f899014ece3d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Coin build for Indonesia 2018 on Bandung";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040bfc9c6f8eb42659d304d078e5c56434eb17646c73a3b90c7adf67cdfd58cd28fb817523b8210e3e1365bc87448f21b56fcac9d66f76ab4ca8eec75bc7df182f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528063923;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087523354;

        if (fTestNet)
        {
            block.nTime    = 1528063672;
            block.nNonce   = 386073234;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x7209f7d9ac59d6f2af70e0aeaeb6cec847b470d693817729bedc44c66ae5954f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Jipcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JipcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JipcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JipcoinMiner(CWallet *pwallet)
{
    printf("JipcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jipcoin-miner");
    ...
    try { loop {
        ...
        printf("Running JipcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JipcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JipcoinMiner, pwallet));
}
